...
void UWeaponComponent::FireVisualImplementation(const FVector& BulletStart, const FVector& HitLocation)
{
	...
	if(UAGPGameInstance* instance = Cast<UAGPGameInstance>(GetWorld()->GetGameInstance()))
	{
		if(!Cast<APawn>(GetOwner())->IsLocallyControlled())
		{
			instance->PlayGunshotSoundAtLocation(BulletStart);
			instance->SpawnGroundHitParticles(HitLocation);
		}
		else
		{
			instance->PlayGunshotSound2D();
			instance->SpawnGroundHitParticles(HitLocation);
		}
